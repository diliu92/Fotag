...
function createModelModule() {
    ...
    var ImageModel = function(
        ...
        this.listeners = [];
        this.notify = function(imageModel, eventTime) {
    ...
    _.extend(ImageModel.prototype, {
        ...
        setCaption: function(caption) {
            if (this.caption != caption){
                ...
                this.notify(this, this.modificationDate);
            }
        },
    ...
    var ImageCollectionModel = function() {
        ...
        this.notify = function(eventType, imageModelCollection, imageModel) {
            _.each(this.listeners, function(listener) {
               listener(eventType, imageModelCollection, imageModel, new Date());
            });
        }
    };
    ...
    _.extend(ImageCollectionModel.prototype, {
        ...
        addImageModel: function(imageModel) {
            var that = this;
            var index = that.imageModels.length;
            this.imageModelListener = function(imageModel, eventTime){
                that.imageModels[index] = imageModel;
                that.notify(IMAGE_META_DATA_CHANGED_EVENT, that, imageModel);
                storeImageCollectionModel(that);
            }
            imageModel.addListener(this.imageModelListener);
            this.imageModels.push(imageModel);
            this.notify(IMAGE_ADDED_TO_COLLECTION_EVENT, this, imageModel);
        },
